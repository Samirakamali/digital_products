...
class User(AbstractBaseUser, PermissionsMixin):
    username = models.CharField(
        ...
        max_length=32,
        unique=True,
        help_text=_('Required. 30 characters or fewer starting with a letter, digits'),
        validators=[
            validators.RegexValidator(r'^[a-zA][a-zA-Z0-9_\.]+$', _('Enter a valid username starting with a-z.'), 'invalid'),
        ],
    ...
    phone_number = models.BigIntegerField(
        ...
        null=True,
        blank=True,
        validators=[validators.RegexValidator(r'^989[0-3,9]\d{8}$', _('Enter a valid mobile number.'), 'invalid')],
        error_messages={'unique': _("A user with this mobile number already exists.")}
    )
    is_staff = models.BooleanField(_('staff status'), default=False)
    is_active = models.BooleanField(_('active'), default=True)
    date_joined = models.DateTimeField(_('date joined'), null=True, default=timezone.now)
...
class UserProfile(models.Model):
    ...
    gender = models.DateTimeField(_('created time'), auto_now_add=True, null=True)
...
class Device(models.Model):
    ...
    users = models.ForeignKey(User, related_name='devices', on_delete=models.CASCADE)
    ...
    class Meta:
        ...
        unique_together = ('users', 'device_uuid')
